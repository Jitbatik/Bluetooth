...
class EventHandlerImpl @Inject constructor() : EventHandler {
    ...
    override fun handleEvent(event: HomeEvent): ByteArray = when (event) {
    ...
    private fun mapButtonToCommand(type: ButtonType): IntArray {
        return when (type) {
            ...
            ButtonType.PLUS -> baseLift + intArrayOf(0x00, 0x02)
            ButtonType.MINUS -> baseLift + intArrayOf(0x00, 0x01)
        }
    }
